...
def draw(
    ...
    *,
    ...
    # One stable opaque header. Keep it compact so it fits the proportional font.
    ...
    # Two large live-rate cards.
    send_cus(
        ser,
        panel(
            0, 27, 238, 35,
            f"RX  {fmt_rate(rx_rate)}  {fmt_count(int(rx_pps))}pps",
            fg=rx_fg,
            bg=rx_bg,
            align="CENTER",
            border=1,
    ...
    send_cus(
        ...
        panel(
            242, 27, 238, 35,
            f"TX  {fmt_rate(tx_rate)}  {fmt_count(int(tx_pps))}pps",
            fg=tx_fg,
            bg=tx_bg,
            align="CENTER",
            border=1,
    ...
    # Fixed columns; no tabs or space-based alignment.
    ...
    table_h = 108
    columns = (
        panel(0,   table_y, 155, table_h, "\n".join(metric_lines), fg=DIM,   bg=BG, align="RIGHT"),
        panel(155, table_y, 130, table_h, "\n".join(rx_lines),     fg=GREEN, bg=BG, align="RIGHT"),
        panel(285, table_y, 130, table_h, "\n".join(tx_lines),     fg=CYAN,  bg=BG, align="RIGHT"),
    )
    ...
    # Footer with socket counts, kept separate from the table.
    footer = f"TCP  EST {established}   LISTEN {listen}"
    send_cus(
        ser,
        panel(0, 174, 480, 26, footer, fg=WHITE, bg=BG, align="CENTER"),
    )
...
def main() -> int:
    ...
    print("480x200; absolute; overlays estaveis; Ctrl+C para sair")
